...
### Normally at the beginning of a script we load in packages. We loaded "tidyverse" above.
...
### Let make a table of the number of captures at each station. What is the $ doing?

```{r}
table(fish_encounters$station)
```

### Now let's make a histogram of the same data (counts of number of fish per station). Note that the two ways of writing the plot code below produce identical results (you can use whichever you prefer):

```{r}
fish_encounters |> ggplot(aes(x = ))
```
...
```{r}
capt <- capture_data.csv
```
*1. Copy or type code from earlier in the script to make a table of birds captured in June*

```{r}

```

*2. If instead we wanted to look at the data as a histogram of capture frequencies, what would we do? Can we make a histogram like with the other data (let's try it, even though I'm sure you see where this is going...)*

```{r}

```

6. Since the data is **already formatted as a count (number of species/month)** we can use a bar plot instead:

```{r}
capt |> ggplot(aes(x=Species, y=June)) +
  geom_col()
```
...
PVAdemo <- function(nreps, nyears, Init_N, R_max, K, Flood_prob, Flood_surv){
  ...
  for(rep in 1:nreps){                            # start looping through replicates
    ...
    for(y in 2:(nyears+1)){                                  # loop through years
      ...
      if(runif(1)<Flood_prob) {
        nextyear <- nextyear*Flood_surv       # catastrophe
...
Does this seem like the right timescale for this data? You can go back and *change the value of "nyears"* to tell R to run it for more time or less time. Just remember you have to do 2 things after changing a parameter values (like "nyears"): *re-run the model* and then also *plot the model output again*. 
...
In that case maybe we should present a histogram of final abundances. Let's set "nyears" to 30 years, re-run the chunk under "RUN THE MODEL", and then run the code below:

[R NOTE: This time we aren't making a function to make the plot, we're just making the plot]

```{r}
time_to_ext <- data.frame(timeToExt = colSums(Default != 0))


time_to_ext |> ggplot(aes(x=timeToExt)) +
  geom_histogram() +
...
ext |> ggplot(aes(x=year, y=ext)) +geom_line()
...
ggplot(survivors, aes(x=scenario, y=flood_survivors)) +
  geom_point(shape=1, size=2, col="purple3")  +
...
How would we use the above code on real-world data? Let's try with the data we collected last lab. We have already specified all the functions in R, so we just need to *call he functions* them on our lab data.
