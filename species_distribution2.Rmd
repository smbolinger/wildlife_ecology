...
ggplot() + 
  geom_polygon(data=worldMap, aes(x=long, y=lat, group=group), color="orchid3")
...
If the world map is a little large for your species' distribution, you can always zoom to a continent, a country, a region...
...
# nAm <- worldMap %>% filter(region == "North America") 
# ggplot() +
#   geom_polygon(data=nAm, aes(x=long, y=lat,  group=group)) +
#   # geom_point(data=sp_gps,aes(x=lat, y=long, group=group), color="orchid")
#   geom_point(data=sp_GPS,aes(x=long, y=lat,), color="orchid")

# nAm <- map_data("north")
usMap <- map_data("usa")
usMap %>% ggplot() +
  geom_polygon(data=usMap, aes( x=long, y=lat, group=group)) +
  geom_point(data=sp_GPS[sp_GPS$lat>0,], aes( x=long, y=lat), color="chartreuse4") 
# 
# ggplot(data=usMap, aes(x=long, y=lat, group=group)) + 
#   geom_polygon(color="black", fill="white") +
#   geom_point(data=sp_GPS, aes( x=long, y=lat), color="chartreuse4")
```
